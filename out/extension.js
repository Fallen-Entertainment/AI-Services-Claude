...
async function activate(context) {
    ...
    const intervalMinutes = config.get('contextUpdateIntervalMinutes', 5);
    updateTimer = setInterval(() => {
        updateClaudeContext(homeDir);
    }, intervalMinutes * 60 * 1000);
    // Register manual update command
    context.subscriptions.push(vscode.commands.registerCommand('aisc.updateContext', async () => {
        await updateClaudeContext(homeDir);
...
// ─────────────────────────────────────────────────────────────────────────────
async function updateClaudeContext(homeDir) {
        ...
        if (fs.existsSync(coldPath)) {
            try {
                ...
                if (Array.isArray(coldData) && coldData.length > 0) {
                    ...
                    coldData.slice(0, 5).forEach((entry) => {
